...
public class ShaclValidator {
    ...
    public static void main(String[] args) throws IOException {
		...
		InputStream input = new FileInputStream(pathroute + "/config.properties");
		Properties prop = new Properties();
		prop.load(input);
		args = new String[3];
		args[0] = "/home/rey/eclipse-workspace/ppsr2dcatcs/dcat2.ttl";
		args[2] = pathroute+prop.getProperty("dcatcs.rdf");
		args[1] = pathroute+prop.getProperty("shacl.shapes");
        ...
        Graph model = null;
        if (args.length == 3) {
            model = readBaseModel(formatRdfPath(args[0]));
        ...
        Path path = formatRdfPath(args[1 + offset]);
        Shapes shapes = readSHACL(path);
        logger.info("{} SHACL shapes in total", shapes.numShapes());

        File instances = new File(args[2 + offset]);
    ...
    public static void validate(Graph model, File instanceFile, Shapes shapes, boolean waitForUser) throws IOException {
        ...
        Graph joinedGraph = new Union((model == null) ? Graph.emptyGraph : model, instance);
        logger.info("Total RDF triples: {} (roughly {}+{})", joinedGraph.size(), (model == null) ? 0 : model.size(), instance.size());

        ValidationReport report = org.apache.jena.shacl.ShaclValidator.get().validate(shapes, joinedGraph);
        if (!report.conforms()) {
            logger.error("Validation failed, details below:");
            logger.error("{}", report.getEntries());
        } else {
    ...
    private static Path formatRdfPath(String path) throws MalformedURLException {
        URL pathAsUrl = Paths.get(path).toUri().toURL();
    ...
    public static Graph readInstance(Path path) throws IOException {
        ...
        byte[] jsonLdArr = Files.readAllBytes(path);
